 ...
 *
 * Страховка по времени обязательна. Если Яндекс не ответит — сеть, блокировка,
 * сбой, — форма без неё осталась бы навсегда не отправленной, а кнопка,
 * не реагирующая на нажатие, читается как поломка сайта. Поэтому через
 * несколько секунд отправляем как есть: токена не будет, и сервер пропустит
 * заявку по тому же правилу, что и запросы без JavaScript.
 */
...
window.razilCaptchaInit = function () {
	...
	Array.prototype.forEach.call( boxes, function ( box ) {
		...
		form.addEventListener( 'submit', function ( event ) {
			...
			// Не дождались ответа — отправляем без токена.
 ...
 *
 * Три случая пропускаются намеренно, и все три — осознанный размен
 * в пользу живого посетителя:
 *
 *   1. Капча не настроена. Локальная машина, где констант нет.
 *
 *   2. Токена нет вовсе. Невидимая капча физически не может выдать токен
 *      без JavaScript. Отклонять значило бы закрыть форму для всех,
 *      у кого скрипты выключены или урезаны — на ритуальном сайте это
 *      люди со старых телефонов и из корпоративных сетей. Потерять живое
 *      обращение хуже, чем пропустить заявку, которая всё равно ляжет
 *      черновиком и попадётся модератору. Бот при этом упирается
 *      в остальные четыре уровня.
 *
 *   3. Сервис не ответил за отведённое время. Иначе работа агентства
